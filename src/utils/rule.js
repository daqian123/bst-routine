...
export default {
    ...
    ruleIsDiscountNum(number) {
        ...
        if (!flag) {
            common.showToast("请输入正确的折扣优惠，如8.5折，输入8.5");
        }
        return flag
    },
    ...
    ruleCardNum(num) {
        ...
        if (!flag) {
            common.showToast("请输入正确的身份证号码");
        }
        return flag
    },
    ruleImg(arr) {
        if (!/\.(jpg|jpeg|png|JPG|PNG|GIF|gif)$/.test(arr[0])) {
            common.showToast("图片类型必须是.jpeg,jpg,png,bmp中的一种");
            return false;
        }
        return true
    },
    ruleVideo(video) {
        if (!/\.(wmv|mp4|avi|rmvb|AVI|RMVB|WMV|MP4)$/.test(video)) {
            common.showToast("视频格式必须是.wmv,mp4,avi,rmvb中的一种");
            return false;
        }
        return true
    },
    ...
    ruleMaxUpload(val, tempFiles, arr) {
        if (tempFiles.length + arr.length > val) {
            common.showToast(`最大只能上传${val}张图片`)
            return false
        }
        return true
    },
    ...
    minUpload(arr, val) {
        if (arr.length < 1) {
            val == 1 ? common.showToast("请上传至少一张轮播图片") : common.showToast("请上传至少一张店铺图集")
            return false
        }
        return true
    },
    ...
    ruleFileSize(arr) {
        ...
        arr.forEach(item => {
            if (item.size / 1024 / 1024 >= 18) {
                common.showToast("上传图片大小不能超过18Mb")
                flag = false
            }
        });
        return flag
    ...
    ruleTime(time, type) {
        let date
        if (type == 2) {
            date = `${common.getDayDate(new Date())} ${time}:00`
        } else {
            date = `${time}:00`
        }
        ...
        if ((startTime - minTime) / 1000 / 60 < 30) {
            common.showToast("发车时间不能小于当前时间半小时，请重新选择")
            return false
        }
        return true
    }
